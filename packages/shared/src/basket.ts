...
//
// Relocated from apps/worker/src/basket.ts by module 10 (worker re-exports):
// the activation mapping (doc 10) derives the contract's capPerExec from the
// LARGEST FINAL LEG, which only this function can name — merges can grow a
// leg past its naive pct×amount, and an under-cap would make the contract
// block the worker's own legitimate legs.

import type { BrokerPlanParams } from "./plan-params";
